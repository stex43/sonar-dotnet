...
public class Sample
{
    public void Method()
    {
        ...
        var m = diagnostics.Should().ContainSingle(x => x.Id == "AD0001").Which.GetMessage();
        diagnostics.Should().ContainSingle(x => x.Id == "AD0001").Which.GetMessage().Should()
            .StartWith("Analyzer 'SonarAnalyzer.UnitTest.Rules.SymbolicExecutionRunnerTest+ConfigurableSERunnerCS' threw an exception of type 'SonarAnalyzer.SymbolicExecution.SymbolicExecutionException' with message 'Error processing method: Method ## Method file: snippet1.cs ## Method line: 3,4 ## Inner exception: System.InvalidOperationException: This check is not useful. ##    at SonarAnalyzer.UnitTest.TestFramework.SymbolicExecution.ThrowAssignmentRuleCheck.PostProcessSimple(SymbolicContext context)");
    }
